...
import { usePostedJobHook } from "../hooks/usePostedJobHook";
...
const CloseJobModal: React.FC<CloseJobModalProps> = ({ status }) => {
  const { modalData, isOpen, closeModal } = useModalStore();
  const { isPendingCloseJob, jobStatusFn } = usePostedJobHook(status);
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-end space-x-2">
              ...
              >
                ...
                {!isPendingCloseJob && status === "Active" && "Close job"}
                {!isPendingCloseJob && status !== "Active" && "Reopen job"}
              </Button>
            </div>
          </div>
        </Modal.Body>
