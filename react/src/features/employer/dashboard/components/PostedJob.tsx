    ...
    <>
      <div className="w-full">
        <div className="grid grid-cols gap-y-1">
          <h1 className="font-bold text-textBlack text-2xl">Job Posted</h1>
          <Breadcrumbs className="text-blue-500 text-sm">{items}</Breadcrumbs>
        </div>
        <div className="py-10">
          <Image
            h={300}
          ...
          <div className="flex flex-col py-7 gap-y-4 md:flex-row md:justify-between md:items-center">
            <div className="grid grid-cols gap-y-2">
              <div>
                <div className="grid grid-cols gap-y-1">
                  <Badge variant="light" color={statusData(viewJobData.status)}>
                    {viewJobData.status}
                  </Badge>
                </div>
                ...
                <div className="flex items-center gap-x-3">
                  ...
                  />
                  <span className="text-textGray text-sm">
                    {dayjs(viewJobData.posted_date).format("MMM DD YYYY")}
                  </span>
                </div>
              </div>
              <div className="flex flex-wrap gap-2">
                {viewJobData.tags.map((tag: string, index: number) => (
            ...
            <div className="flex items-center gap-x-3">
              <Button
                ...
                variant={viewJobData.status === "Active" ? "outline" : ""}
          ...
          <div className="grid grid-cols gap-y-9">
            ...
            <div className="grid grid-cols gap-y-3">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:flex gap-2">
                {applicantsStatus.map((appl, index) => (
